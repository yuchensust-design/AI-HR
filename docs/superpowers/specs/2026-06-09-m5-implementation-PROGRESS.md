...
## 〇、Playwright 文字模式端到端实测（2026-06-09，全程跑通）

用 Playwright 开窗、文字模式、严厉性格、半结构化、AI 产品经理简历+JD 跑完整一场并审查复盘：

**验证通过**：
- 出题结合简历且按性格质疑（"主导切 DeepSeek 88%，独立做的吗？怎么评估的？"）
- 动态追问：含糊答案→当场追问（题数 5→8）；好答案→预门跳过；追问不再被追问 ✅
- R1 刷新恢复：headless chrome 中途崩了 2 次，"继续上次面试"每次都恢复到当前题、零丢失 ✅
- 复盘 4 维：evidence 全引真实原话+数字、口水话精确；能力雷达（v5 G1）正常 + **抓出"反思深度 2/5"**（4 维全 5 分看不出的真问题）；highlights 含追问题、bullet 无编造、无大厂名泄漏 ✅
- "查看回答思路"：已是结合简历的针对性提示（50场/88%/光伏53%/非遗AR），不给完整答案 ✅

**实测发现并已修的 2 个 bug**：
- debrief `max_tokens` 3000→6000：动态追问让题数增多+答案长 → 复盘输出超限截断 → JSON 解析失败 → 整页 502（已复现并修）
- 复盘题数显示：配置数(5) ≠ 实际数(8)，改为"5 题 +3 追问" / "8 题完整摘要"

**环境噪声（非产品 bug）**：headless chrome 提交后偶发崩溃（叠加 ASR WS 反复重连）；trace 报 `m5_llm_traces` 表不存在（migration 004 未对 dev 库执行，fire-and-forget 已优雅降级）。
